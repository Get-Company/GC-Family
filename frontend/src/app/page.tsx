...
function QuickTaskOverview({ instances, currentMemberId, isParent, onComplete, onShare, onReopen, onUndo }: { instances: Instance[]; currentMemberId: number | null; isParent: boolean; onComplete: (id: number) => void; onShare: (id: number) => void; onReopen: (id: number) => void; onUndo: (id: number) => void }) {
  ...
  return <section className="rounded-3xl border p-5 sm:p-6" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-background)" }}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-wide" style={{ color: "var(--color-secondary)" }}>Schnellüberblick</p><h2 className="text-3xl font-bold">Offene Aufgaben</h2></div><Link href="/tasks" className="button-secondary">Alle Aufgaben</Link></div><ul className="mt-4 grid gap-3">{open.length > 0 ? open.map((instance) => <TaskCard key={instance.id} instance={instance} onComplete={onComplete} onShare={onShare} onReopen={isParent ? onReopen : undefined} onUndo={onUndo} canUndo={Boolean(currentMemberId && instance.contributions.some((contribution) => contribution.member_id === currentMemberId))} showCompletionDetails />) : <li className="rounded-2xl border border-dashed p-6 text-center" style={{ borderColor: "var(--color-border)" }}>Alles erledigt – super!</li>}</ul></section>;
}
...
function isCurrentWeek(date: string) { const { start, end } = currentWeekBounds(); const day = new Date(`${date}T12:00:00`); return day >= start && day <= end; }
