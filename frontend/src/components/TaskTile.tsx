  ...
  return <li className={`task-tile ${task.available ? "" : "task-tile-inactive"}`}>
    ...
    {expanded && <div id={`task-details-${task.id}`} className="task-details relative border-t p-3 sm:p-5" style={{ borderColor: "var(--color-border)" }}>
      ...
      {memberId === null ? <a href="#family-pin" className="button-secondary mt-3 w-full text-xs">Zum Abhaken anmelden</a> : <div className="mt-3 flex flex-col gap-2">
        {canComplete && <button type="button" disabled={pending} onClick={() => void onAction(task, alwaysAvailable ? "always_complete" : "complete")} className="button-primary button-complete text-sm"><span className="complete-button-check" aria-hidden="true">✓</span><span>{pending ? "Speichert…" : alwaysAvailable && task.completion_count_today ? "Noch einmal abhaken" : "Abhaken"}</span><span className="complete-button-sparkle" aria-hidden="true">✦</span></button>}
