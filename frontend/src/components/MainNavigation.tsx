...
function NavButton({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} aria-label={label} title={label} className="flex h-11 w-11 shrink-0 flex-col items-center justify-center rounded-xl border text-[8px] font-bold leading-none tracking-tight transition-colors focus-visible:outline-none focus-visible:ring-2" style={{ borderColor: "var(--color-border)", color: "var(--color-foreground)", backgroundColor: "var(--color-button-surface)" }}><span className="mb-0.5">{children}</span><span>{label}</span></button>;
}
